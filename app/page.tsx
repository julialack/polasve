...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f8f9fa]">
      ...
      <div className="bg-white border-b border-zinc-200 py-3 px-6 text-left">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-6 py-8 text-left">
        <div className="grid grid-cols-1 md:grid-cols-12 lg:grid-cols-4 gap-6 text-left">
          ...
          <aside className="hidden md:block md:col-span-3 lg:col-span-1 space-y-6 text-left">
            <section className="bg-white shadow-sm overflow-hidden border border-zinc-200 rounded-sm text-left">
              <div className="bg-[#a11a2d] text-white px-4 py-2 text-xs font-bold uppercase tracking-wider text-left">Information</div>
              <div className="p-5 text-left">
                <p className="text-[11px] text-zinc-600 leading-relaxed italic font-serif text-left">&quot;Samlingsplatsen för polska nätverk i Sverige. Upptäck möjligheter och gemenskap.&quot;</p>
              </div>
            </section>
               ...
               <nav className="flex flex-col space-y-1 text-left">
                {[
                  ...
                  <Link key={item.name} href={item.href} className="flex items-center justify-between py-2.5 hover:text-red-800 text-[11px] font-bold text-[#003366] border-b border-zinc-50 last:border-0 transition-colors group text-left">
                    <div className="flex items-center gap-3 text-left">{item.icon}{item.name}</div>
          ...
          <div className="col-span-1 md:col-span-9 lg:col-span-2 space-y-6 text-left">
            ...
            <section className="bg-white border border-zinc-200 shadow-sm overflow-hidden text-left">
              ...
              <div className="p-4 text-left">
                {news.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
                    <Link href="/nyheter" className="group text-left block">
                      <div className="aspect-video relative overflow-hidden rounded-sm border text-left">
                        <img src={news[0].image_url || "https://images.unsplash.com/photo-1517048676732-d65bc937f952?q=80&w=800&auto=format&fit=crop"} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                      </div>
                    ...
                    <div className="space-y-4 text-left">
                      {news.slice(1).map((item) => (
                        <Link key={item.id} href="/nyheter" className="flex gap-3 group border-b border-zinc-50 pb-2 last:border-0 text-left">
                          <div className="w-12 h-12 relative flex-shrink-0 rounded-sm overflow-hidden border text-left">
                            <img src={item.image_url || "https://images.unsplash.com/photo-1519389950473-47ba0277781c?q=80&w=200&auto=format&fit=crop"} alt="" className="w-full h-full object-cover" />
                          </div>
                ...
                ) : <p className="text-center py-10 text-zinc-400 italic text-xs text-left">Väntar på dagens nyheter...</p>}
              </div>
            </section>
            ...
            <section className="bg-white border border-zinc-200 shadow-sm overflow-hidden text-left">
              <div className="bg-[#003366] text-white px-4 py-2 text-xs font-bold uppercase tracking-wider flex justify-between items-center text-left">
                <span className="text-left">Community Flöde - Realtid</span>
                  ...
                  <Link href={`/annonser/${ad.id}`} key={ad.id} className="block group border-b border-zinc-100 last:border-0 pb-4 text-left">
                    <div className="flex gap-4 text-left">
                      <div className="w-16 h-16 bg-zinc-100 relative overflow-hidden border rounded-sm text-left">
                        <img src={ad.image_url || "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?q=80&w=200&auto=format&fit=crop"} alt="" className="w-full h-full object-cover" />
                      </div>
                      <div className="text-left flex-1 min-w-0 text-left"><h4 className="text-[11px] font-bold text-[#003366] italic leading-tight text-left truncate">{ad.title}</h4><p className="text-[10px] text-red-800 font-bold mt-1 text-left">{ad.price}</p></div>
                    </div>
                  </Link>
                ))}
              </div>
