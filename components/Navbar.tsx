...
export default function Navbar() {
  ...
  return (
    <header className="bg-white border-b border-zinc-100 z-50 shadow-sm relative">
      {/* Top Part: Centered Logo & User Actions */}
      <div className="py-4 md:py-8 flex justify-center items-center relative px-4 md:px-6">
        {/* Mobile Hamburger (Visible on small screens only) */}
        <button
          className="md:hidden absolute left-4 p-2 text-zinc-600 hover:bg-zinc-50 rounded-md transition-colors"
          onClick={() => setMenuOpen(!menuOpen)}
        >
          {menuOpen ? <X size={24} /> : <Menu size={24} />}
        </button>

        {/* Logo (Always centered on Desktop, middle on Mobile) */}
        ...
        {/* Desktop Profile & Messages Icons (Right side) */}
        <div className="hidden md:flex absolute right-6 lg:right-10 items-center gap-4 lg:gap-6">
          {user ? (
            <>
      ...
      <nav className="border-t border-zinc-100 hidden md:block bg-zinc-50/30">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <li key={link.name} className="relative">
                  <Link
                    href={link.href}
                    className={`block py-5 text-[11px] lg:text-[13px] font-black uppercase tracking-[0.2em] transition-colors ${
